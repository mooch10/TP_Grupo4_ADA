# Tp_Grupo4 — Ecosistema de Microservicios (Java 21 + Spring Boot)

Descripción: proyecto de microservicios con Spring Boot y Maven organizado en múltiples módulos. Está preparado para ejecución local (IDE / `mvn spring-boot:run` o `java -jar`).
...
| **Elasticsearch** | 9200 | Almacenamiento y búsqueda de logs (local) |
| **Logstash** | 5044 | Recolección y parseo de logs (instalación local) |
| **Kibana** | 5601 | UI para visualizar logs (local) |

## Requisitos Previos

### Requisitos para ejecución local (recomendado):
- Java 21
- Maven 3.9+
- [Erlang/OTP](https://www.erlang.org/downloads) (para RabbitMQ) o broker Kafka instalado
...
<!-- Se han eliminado las instrucciones específicas sobre contenedores. El repositorio está preparado para ejecución local; seguir la sección "Arranque manual (local)" para pasos detallados. -->

## Arranque manual (local)

Si tienes poca memoria RAM, puedes ejecutar todo localmente usando instalaciones nativas. Requiere más configuración inicial pero consume menos recursos.
...
Seleccioná el broker que quieras usar cambiando el Spring Profile a `rabbitmq` o `kafka`. El cambio de broker **no requiere modificar código**, solo ajustar el profile en el arranque de los servicios.
...
- **Kafka**: No tiene UI incluida. Usá la utilidad nativa `kafka-topics.sh` del binario para listar topics:

```bash
# Desde el binario de Kafka instalado localmente:
...
|-----------|--------|---------|---------------|
| **Elasticsearch** | 9200 | Almacena y permite buscar logs | Local opcional (configuración incluida) |
| **Logstash** | 5044 | Recibe logs de los servicios, los parsea y los envía a Elasticsearch | Instalación local (Windows) |
| **Kibana** | 5601 | UI web para buscar, filtrar y visualizar logs | Local opcional (configuración incluida) |

<!-- Instrucciones Docker eliminadas: seguir la sección de Arranque manual (sin Docker) o instalar Elasticsearch/Kibana localmente según la documentación oficial -->
...
```bash
# Ejecutar Logstash con el pipeline del proyecto (ajustar ruta según tu instalación)
cd "C:\Program Files\logstash-8.17.0"
bin\logstash.bat -f "<ruta-al-repositorio>\logstash\pipeline.conf"
```
...
1. Si vas a usar ELK localmente, arrancá **Elasticsearch** y **Kibana** siguiendo sus guías oficiales y verificá que respondan en `http://localhost:9200` y `http://localhost:5601`.

2. Iniciá **Logstash** con el pipeline incluido (si lo usás):
...
3. Arrancá los servicios Spring Boot en este orden (localmente, sin Docker): `config-server`, `eureka-server`, `auth-service`, `inventory-service` (perfil `rabbitmq` o `kafka`), `notification-service`, `api-gateway`.
...
- Revisá los logs del servicio de Elasticsearch según tu instalación (Windows: carpeta de logs de la instalación; Linux: `journalctl -u elasticsearch` o `cat /var/log/elasticsearch/*`).
- Si usás una instalación nativa, consultá la documentación oficial para resetear datos de índice o limpiar directorios de datos.
